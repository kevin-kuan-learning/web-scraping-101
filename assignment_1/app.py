...
item_info = []
item_node_list = tree.xpath(
    "//div[@itemscope='itemscope' and @data-listing-id]")
for node in item_node_list:

    title = node.xpath(".//h3//span[@itemprop='name']/text()")[0]
    price = node.xpath(".//span[@itemprop='price']/text()")[0]
    url = node.xpath(
        ".//div[@class='dne-itemtile-detail']/a[@itemprop='url']/@href")[0]
    ...
    try:
        discount_node = node.xpath(
            ".//span[contains(@class, 'itemtile-price-bold')]/text()")[0]
